...
```{r}
library(lmtest)
model1 = lm(Avg.sleep.time ~ Age + Game.3 + Avg.hours.exercise, data = df)
summary(model1)
...
ggplot(df, aes(x = Age, y = Avg.sleep.time)) +
  geom_point(aes(color = "blue"), size = 3) +
  geom_point(data = df[53,], aes(x = Age, y = Avg.sleep.time), color = "red", size = 3) +
  geom_point(data = df[133,], aes(x = Age, y = Avg.sleep.time), color = "green", size = 3) +
  geom_point(data = df[88,], aes(x = Age, y = Avg.sleep.time), color = "black", size = 3) +
  scale_color_manual(values = c("blue" = "blue", "red" = "red","green"="green", "black" = "black")) +
  theme_minimal() +
  labs(title = "Scatter Plot")
...
```
## add age to model remove sport compare both then use aic for one model and bic for the other
```{r}

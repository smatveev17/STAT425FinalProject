...
```{r}
library(dplyr)
df$Inverse.Reaction.time <- (1/sqrt(df$Reaction.time))

model1_updated =lm(Inverse.Reaction.time~ Avg.hours.exercise + Avg.sleep.time + Age+ Noise.level,data=df)
...
```{r}
backward_model = lm(Inverse.Reaction.time~Avg.hours.exercise+Avg.sleep.time+Age, data=df)
...
```{r}
model2 = lm(Inverse.Reaction.time~ Avg.hours.exercise+ Age+Noise.level+Avg.sleep.time*Input.type,data=df)
summary(model2)
```
...
```{r}
anova(model1_updated,model2)
```
H0: The reduced model is sufficient

H1: We require the full model

Since p-value is less than .05, we require the full model with the interaction term
...
Based on the AIC criteria, the best model is model 2 which has the interaction term
## Best BIC model
```{r}
BIC(model1_updated)
```
...
Based on the BIC criterion, the best model is the backward model which makes sense as BIC criterion is more conservative and prefers smaller models due to the selection parameter of nlog(n)
